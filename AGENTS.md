...
> 役割：**発火時に「今のスロット用の投稿を1本」生成し、必要なら GPT 画像も作り、Threads に投稿してリポジトリを更新する**。
...
依存：`pip install requests openai`（poster は requests、画像生成は openai を使う）。
...
投稿タイプ：`単発` を基本、たまに `コメント仕込み`（本文中で `--- コメント1 ---` 区切り）。**画像付き**は §3 のとおり時々入れてよい。

## 3.（画像付きにする場合）GPT-Image で画像生成
1. 画像プロンプトを作り、**`gpt-image-1`** で **PNG・正方形 `1024x1024`・5MB以下**を生成（文字は入れない／クリーンな構図）。
2. 保存先＝**`storage/images/<投稿ID>.png`**（ファイル名は投稿IDと完全一致・小文字 `.png`）。
3. 本文ブロックに **`- 画像ファイル: storage/images/<投稿ID>.png`** 行を必ず入れる。
4. **⚠️ Threads は画像を公開URL（`raw.githubusercontent.com/.../main/storage/images/<ID>.png`）で添付する。**
...
```bash
cd <repo>
SKIP_JITTER=1 python scripts/threads_poster.py
```
- 認証は §0 の環境変数から自動取得。`SKIP_JITTER=1` で遅延を切る。
- **画像付きの場合は §3-4 の事前 push を済ませてから実行**（でないと画像URLが404）。
