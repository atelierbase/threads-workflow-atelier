#!/usr/bin/env python3
"""Queue one image2-generated Threads post in pending.md.
...
def validate_post(args: argparse.Namespace, text: str, image_prompt: str) -> list[str]:
    ...
    if not text.strip().endswith(("?", "？")):
        errors.append("Threads text must end with a question")
    ...
    for i, segment in enumerate(parse_segments(text), 1):
        if len(segment) > 500:
            errors.append(f"text segment {i} exceeds 500 chars: {len(segment)}")
...
def append_pending(args: argparse.Namespace, text: str, image_prompt: str) -> None:
    ...
    if not PENDING.exists():
        PENDING.write_text("# 受け渡しキュー（Threads / image2生成済み画像 / 定期投稿）\n\n---\n", encoding="utf-8")
...
def main() -> int:
    ...
    print(f"queued image2 Threads post {args.post_id} for slot {args.slot}")
    return 0
